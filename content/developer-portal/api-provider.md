...
). This guide explains how to configure custom rate limit keys.
...
) defined in a developer profile as metadata fields to credentials.
...
Go directly to [Configure Tyk Enterprise Developer Portal to work with an identity provider](#configure-tyk-enterprise-developer-portal-to-work-with-an-identity-provider).
 ...
 ). Access rights and rate limits defined in the default policy take priority over other policies, including policies for the API Product and plan.
...
). If your Identity Provider uses certificates that are not trusted, the portal will not work with it by default. To bypass certificate verification, you can select the `SSL secure skip verify` checkbox.
...
). If you need to specify more than one scope, you can separate them with spaces.

) section of this guide to associate them with that product.
...
) if you wish to change the email template.
