...
To upgrade the portal's theme please follow the [upgrade instructions](/portal/customization#upgrading-themes) for the portal's themes.
...
<h4 id="Changelog-v1.13.1">Changelog</h4>
#### Fixed 
...
<h4 id="breaking-changes-v1.13.0">Breaking Changes</h4>
...
<h4 id="Changelog-v1.13.0">Changelog</h4>
##### Added
<ul>
<li>
<details>
...
<h4 id="Changelog-v1.12.0">Changelog</h4>
...
You can read more about the supported SSO providers [here](/tyk-identity-broker).
...
- Added new TLS variables to set MinVersion ([portal_tls_min_version](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_min_version), MaxVersion ([PORTAL_TLSCONFIG_MAXVERSION](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_max_version), and CipherSuites ([PORTAL_TLS_CIPHER_SUITES](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_tls_cipher_suites).
...
- Added an assets cache for improved performance on database-backed themes. This speeds up the portal's pages loading time by 30%. It's enabled by default and you can disable using [PORTAL_ASSETS_CACHE_DISABLE](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_assets_cache_disable).
- Added three new configuration options to manage database connections lifecycle: [PORTAL_DATABASE_MAX_OPEN_CONNECTIONS](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_open_connections), [PORTAL_DATABASE_MAX_IDLE_CONNECTIONS](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_max_idle_connections), and [PORTAL_DATABASE_CONNECTION_MAX_LIFETIME](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_database_connection_max_lifetime).
...
If you are on 1.8.3 or older version please follow the [upgrade instructions](/portal/customization#upgrading-themes) to upgrade the portal's themes.
...
- Changed the credential provisioning flow to automatically include DeveloperID, OrganizationID, ApplicationID, and TeamIDs in [the credential metadata](/portal/customization#default-attributes-of-user-model).
- Added warning regarding potential PII exposure to the [custom attributes menu](/portal/customization#default-attributes-of-user-model).
...
- Changed the behavior of the `Secure` cookie attribute that is set by [PORTAL_SESSION_SECURE](/product-stack/tyk-enterprise-developer-portal/deploy/configuration#portal_session_secure) so that the `Secure` attribute is always add to the `Set-Cookie` header whenever `PORTAL_SESSION_SECURE` is set to `true` or when TLS is enabled.
...
- Extended [filesize limit](/portal/customization#create-a-theme) for individual files in themes to 5 MB.
- Made the organization invite email's subject configurable via [the emails settings section](/portal/customization#supported-email-notifications).
...
- When the portal theme [manifest](/portal/customization#manifest-file) has a reference to a template that is not present in the theme then the theme won't be loaded. This check that prevents admin users from uploading themes with potential errors was introduced in version [1.7.0](/#content-blocks-validation).
...
<img src="img/dashboard/portal-management/enterprise-portal/1.8.0-create-custom-attribute.png" alt="Create a custom attribute for the User model" />

This is how it looks like in the user sign-up form:
<img src="img/dashboard/portal-management/enterprise-portal/1.8.0-sign-up-form.png" alt="The user sign-up form with the custom attribute" />
...
To set the CORS configuration please refer to the Portal's [configuration documentation](/product-stack/tyk-enterprise-developer-portal/deploy/configuration.md#cors-settings).
...
<img src="img/dashboard/portal-management/enterprise-portal/1.8.0-test-idp-connectivity.png" alt="Test connectivity to an IdP" />
...
Now when the [PORTAL_DCR_LOG_ENABLED](/product-stack/tyk-enterprise-developer-portal/deploy/configuration.md#portal_dcr_log_enabled) environment variable is set to `true`, the portal will output not only the status and status code of the request to the IdP, but also actual payload returned by the IdP: 
```yaml
...
- When [PORTAL_DCR_LOG_ENABLED](/product-stack/tyk-enterprise-developer-portal/deploy/configuration.md#portal_dcr_log_enabled) is enabled the portal now returns not only the status and status code of the request to the IdP but also actual payload returned by the IdP
...
- Fixed the bug where the [Forgot Password page](/portal/api-consumer#reset-password) did not reflect the current theme.
...
With this new feature, customers can simply use the same SQL database (MySQL, MariaDB and PostgreSQL) for both assets and metadata. To use the `db` [storage type](/product-stack/tyk-enterprise-developer-portal/deploy/configuration.md#portal_storage) just set the `PORTAL_STORAGE=db` for environment variables or `"Storage": "db"` in a config file and you are good to go!
...
API Providers can [configure Single Sign-on](/portal/settings#configure-developer-portal-sso) for the Enterprise developer portal so that it's possible to login developers and admins to the portal user 3rd party IdP.
...
- Added [Single Sign-on](/portal/settings#configure-developer-portal-sso) for API Consumers and admin users so that they can use their IdPs for managing admin users and developers.
...
Now API Consumers can [create organizations](/portal/api-consumer#manage-api-consumer-organizations) and securely share credentials between their teammates. In greater detail:
...
##### Get started guides
API Providers can add [Get started guides](/portal/api-provider#documentation-for-your-api-products) to API Products for better developer experiences:
...
API Providers can select which blogs posts to display on an API Product page using [the tags feature](/portal/api-provider#documentation-for-your-api-products). To achieve that, an API Provider can specify tags for both API Products and blog posts. Blog posts that match tags with an API Product are displayed in the 'Related blog content' section in the API Product page. This offers API Providers greater control over what blog posts to display on their API Product page.
...
##### Added
- Added the [organization management capability](/portal/api-consumer#manage-api-consumer-organizations) for API Consumers to safely share API access credentials between team members.
- Added the [Get started guides](/portal/api-provider#documentation-for-your-api-products) for API Products so that admins can explain to their consumers how use their API Products.
...
- Added [tags](/portal/api-provider#documentation-for-your-api-products) for API Products and blog posts so that API Providers have greater control over which blog posts to display on their API Product page.
